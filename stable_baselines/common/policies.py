...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
        ...
        with tf.variable_scope("input", reuse=False):
            self._dones_ph = tf.placeholder(tf.float32, (None, ), name="dones_ph")  # (done t-1)
            state_ph_shape = (None, ) + tuple(state_shape)
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, layers=None,
        ...
        if net_arch is None:  # Legacy mode
            ...
            with tf.variable_scope("model", reuse=reuse):
                ...
                input_sequence = [extracted_features]
                masks = [tf.reshape(self.dones_ph, [-1, 1])]
