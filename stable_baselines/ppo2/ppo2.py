...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 full_tensorboard_log=False, n_envs=n_envs, observation_space=observation_space, action_space=action_space):
    ...
    def _get_lstm_pretrain_placeholders(self):
        policy = self.act_model
        # if isinstance(self.action_space, gym.spaces.Discrete):
        #     return policy.obs_ph, self.action_ph, policy.policy
        return policy.obs_ph, self.action_ph, policy.dones_ph, policy.states_ph, policy.deterministic_action
           ...
           for epoch_idx in range(int(n_epochs)):
               ...
               for _ in range(dataset.max_train_traj_length):
                   ...
                   feed_dict = {
                       ...
                       dones_ph: expert_dones,
                       states_ph: states,
                   }
                   states, train_loss_, _ = self.sess.run([self.act_model.snew, loss, optim_op], feed_dict)
                   for idx, done in enumerate(expert_dones):
                       if done:
                           states[idx] = np.zeros(states[idx].shape) 
                   train_loss += train_loss_

               train_loss /= dataset.max_train_traj_length
               ...
               if self.verbose > 0 and (epoch_idx + 1) % val_interval == 0:
                   ...
                   for _ in range(dataset.max_train_traj_length):
                       ...
                       states, val_loss_ = self.sess.run([self.act_model.snew, loss], feed_dict)
                       for idx, done in enumerate(expert_dones):
                           if done:
                               states[idx] = np.zeros(states[idx].shape)
                       val_loss += val_loss_

                   val_loss /= dataset.max_val_traj_length
